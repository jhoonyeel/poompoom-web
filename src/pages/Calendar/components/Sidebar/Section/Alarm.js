...
export default function Alarm({ posts, events }) {
  ...
  useEffect(() => {
    ...
    const detectChanges = (prevItems = [], currentItems = [], type) => {
      // currentItems와 prevItems가 배열인지 확인
      ...
      safeCurrentItems.forEach((item) => {
        ...
        if (!prevItem) {
          changes.push({
            type,
            changeType: 'created',
            message: `${type === 'post' ? '게시글' : '일정'}을 작성했습니다.`,
            createdAt: new window.Date(),
          });
        } else if (
          JSON.stringify(prevItem.content) !== JSON.stringify(item.content) // 이전 상태와 현재 상태 비교
        ) {
          changes.push({
            type,
            changeType: 'updated',
            message: `${type === 'post' ? '게시글' : '일정'}을 수정했습니다.`,
            createdAt: new window.Date(),
          });
        }
      });
      ...
      safePrevItems.forEach((prevItem) => {
        if (!safeCurrentItems.find((item) => item.createdAt === prevItem.createdAt)) {
          changes.push({
            type,
            changeType: 'deleted',
            message: `${type === 'post' ? '게시글' : '일정'} 을 삭제했습니다.`,
            createdAt: new window.Date(),
          });
        }
      });
    ...
    const flattenedPosts = Object.values(posts || {}).flat(); // posts의 이중 배열을 단일 배열로 변환
    ...
    const updatedLogs = [...postChanges, ...eventChanges];
